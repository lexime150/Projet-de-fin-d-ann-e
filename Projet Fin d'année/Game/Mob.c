...
void LoadMob(void)
{
	...
	{
		AddMob(ZOMBIE, GetMushroomSpawn(i).x, GetMushroomSpawn(i).y);
	}
	...
	{
		AddMob(ZOMBIE, GetEnemySpawn(i).x, GetEnemySpawn(i).y);
	}
	if (mobCount > 0)
	{
...
void AddMob(TypeMob _type, float _x, float _y)
{
	...
	case ZOMBIE:
		...
		newMob.shape.rect = CreateRectangle((sfVector2f){ ZOMBIE_SIZE, ZOMBIE_SIZE }, (sfVector2f){ZOMBIE_SIZE / 2, ZOMBIE_SIZE}, (sfVector2f){GAME_SCALE, GAME_SCALE}, sfBlue);
		newMob.shape.collisionRect = CreateRectangle((sfVector2f){ 24.f, 32.f}, (sfVector2f){12.f, 32.f}, (sfVector2f){GAME_SCALE}, sfGreen);
		...
		sfRectangleShape_setSize(newMob.shape.attackRect, (sfVector2f){48, 28});
		sfRectangleShape_setOrigin(newMob.shape.attackRect, (sfVector2f){24, 28});
