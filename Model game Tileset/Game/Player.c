...
void MovePlayer(float _dt)
{
	...
	if (!sfKeyboard_isKeyPressed(sfKeyD) && !sfKeyboard_isKeyPressed(sfKeyQ) && !sfKeyboard_isKeyPressed(sfKeySpace) && player.currentState != IDLE && player.isGrounded && !player.isAttacking)
	{
		StateMachine(IDLE);
	...
	if (sfKeyboard_isKeyPressed(sfKeyH) && !player.isAttacking)
	{
		StateMachine(DASH);
